...
char *merge(int offset, int zeros) {
    char *soffset = malloc(offset == 0 ? 1 : (int) log10(offset) + 1);
    char *szero = malloc((int) log10(zeros) + 1);
    sprintf(soffset, "%d", offset);
    sprintf(szero, "%d", zeros);
    return strcat(strcat(soffset, " "), strcat(szero, "\n"));
}

bool checkzero(char buf[], int size) {
    ...
    while (bytesRead != 0) {
        ...
        if (checkzero(buf, bytesRead)) {
            ...
            if (zerocount > 0) {
                ...
                char *strtowrite = merge(offset, zerocount);
